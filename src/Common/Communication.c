...
void *safe_zmq_ctx_new(gful_lifo **graceful_lifo){
    ...
    createContextDataforClosing(zmq_close, zmq_ctx, graceful_lifo);
    return zmq_ctx;
}
...
uint8_t join_request(void * s_, Client sendMessage, gful_lifo **gracefull_lifo){
    ...
    //Prepare and send
        ...
        sendMessage.ch.data = &msg;
        sendMessage.ch.len = 1;

        int msg_len = client__get_packed_size(&sendMessage);
        ...
        safe_zmq_send(s_, buffer, msg_len, 0, gracefull_lifo);

    safe_zmq_recv(s_, buffer, msg_len, 0, gracefull_lifo);

    //Handle received message 
        Client *recvMessage = client__unpack(NULL, msg_len, buffer);
        closeSingleContext(gracefull_lifo);
        msg = *(recvMessage->ch.data);

        client__free_unpacked(recvMessage, NULL);
